...
flowchart TD
  ...
  subgraph host [Aether Host Application - Linux]
    API[JSON-RPC API]
    
    subgraph interactive [Interactive Threads]
      TGM[**Transform Graph Manager**]
      COMM_MGR1[**Connection Manager**<br/>EtherCAT]
      COMM_MGR2[**Connection Manager**<br/>Printer]
      COMM_MGRN[**Connection Manager**<br/>Other]
    end

    subgraph realtime [Hard Realtime Threads]
      TGE[Transform Graph Executor]
      COMM_RT1[**RT Connection**<br/>EtherCAT]
      COMM_RT2[**RT Connection**<br/>Printer]
      COMM_RTN[**RT Connection**<br/>Other]
    end
  end

  MCU1[EtherCAT components]
  MCU2[Aether Printer MCU]
  MCUN[Other proprietary system]

  UI <-.->|JSON-RPC| API
  API -.->|configure| TGM
  TGM <-->|execution plans| TGE
  TGE -->|fill transactions| COMM_RT1
  TGE -->|fill transactions| COMM_RT2
  TGE -->|fill transactions| COMM_RTN
  COMM_MGR1 -.->|manages| COMM_RT1
  COMM_MGR2 -.->|manages| COMM_RT2
  COMM_MGRN -.->|manages| COMM_RTN
  COMM_RT1 ===>|EtherCAT| MCU1
  COMM_RT2 ===>|USB| MCU2
  COMM_RTN ===>|Ethernet| MCUN

  classDef interactive fill:#e1f5ff,stroke:#0288d1
  ...
  class TGM,COMM_MGR1,COMM_MGR2,COMM_MGRN interactive
  class TGE executor
  class COMM_RT1,COMM_RT2,COMM_RTN realtime
  class MCU1,MCU2,MCUN hardware
  class API api
```
